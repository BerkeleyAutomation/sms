...
class smsDataset(InputDataset):
    ...
    def add_image(self,img,depth,cam, detic_outputs=None):
        if self.cur_size == 0:
        ...
        if detic_outputs is not None:
            self.detic_outputs.append(detic_outputs)
        self.cur_size += 1
        # import pdb; pdb.set_trace()
    ...
    def __getitem__(self, idx: int):
        ...
        """
        data = {"image_idx": idx, "image": self.image_tensor[idx], "depth": self.depth_tensor[idx], "mask": self.mask_tensor[idx], "detic": self.detic_outputs[idx]}
        return data