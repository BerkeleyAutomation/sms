...
class Trainer:
    ...
    def deproject_to_RGB_point_cloud(self, image, depth_image, camera, detic_out = None, num_samples = 100, device = 'cuda:0'):
        ...
        scale = self.pipeline.datamanager.train_dataparser_outputs.dataparser_scale
        H = self.pipeline.datamanager.train_dataparser_outputs.dataparser_transform
        idf = self.pipeline.datamanager.train_dataparser_outputs.metadata['image_downscale_factor']

        c2w = camera.camera_to_worlds.to(device)
        depth_image = depth_image.to(device)
        image = image.to(device)
        fx = camera.fx.item() * idf
        fy = camera.fy.item() * idf
    ...
    def process_image(self, msg:ImagePose, step, points, clrs, camlr, clip_dict = None, dino_data = None):
        ...
        detic_out = self.pipeline.datamanager.train_dataset.detic_outputs[cidx]
                ...
                with self.train_lock:
                    ...
                    with TimeWriter(writer, EventName.ITER_TRAIN_TIME, step=step) as train_t:
                        ...
                        if all(expain): # and BA_flag:
                            ...
                            # self.pipeline.model.components_new.extend(pop_n_elements(self.components_queue, num_add))
