...
class GaussianVisualizer:
    ...
    def __init__(
        ...
        self.pipeline.group_labels_local, self.pipeline.group_masks_local, self.pipeline.group_masks_global = self.group_labels, self.group_masks, self.group_masks_global
        print("Groups loaded")
        self.pipeline.init_means = self.pipeline.model.gauss_params['means'].detach().clone()
        # self.pipeline.init_means_homog = torch.cat([self.pipeline.init_means, torch.ones((self.pipeline.init_means.shape[0], 1)).to(self.pipeline.device)], dim=1)
        self.pipeline.init_quats = self.pipeline.model.gauss_params['quats'].detach().clone()
