...
from grasp_ply_karim import get_grasps_obj, exec_grasp
...
class ToadObject:
    ...
    def generate_grasps(
        ...
        # do subprocess stuff
        contact_graspnet_env_path = "~/anaconda3/envs/contact_graspnet/bin/python"
        generate_grasps_path = "/home/lifelong/sms/sms/ur5_interface/ur5_interface/scripts/generate_grasps_karim.py"
        obj_only = True
        if obj_only:
            local_regions = False
            filter_grasps = False
        ...
        debug = True
        z_range = [0.2,1.8]
        result = subprocess.run([contact_graspnet_env_path, generate_grasps_path, points, obj_points, colors, 
                                local_regions, filter_grasps, forward_passes, debug, z_range], capture_output=True, text=True)
