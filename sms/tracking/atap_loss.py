...
class ATAPLoss:
    ...
    def __init__(self, sms_model: smsGaussianSplattingModel, group_masks: List[torch.Tensor], group_labels: torch.Tensor, dataset_scale: float = 1.0):
        """
    ...
    def __call__(self, connectivity_weights: torch.Tensor):
        ...
        wp.launch(
            ...
            kernel = atap_loss,
            inputs = [wp.from_torch(self.sms_model.gauss_params['means'],dtype=wp.vec3),wp.from_torch(self.dists),
    ...
    def _radius_nn(self, group_mask: torch.Tensor, r: float):
        ...
        """
        global_group_ids = torch.zeros(self.sms_model.num_points,dtype=torch.long,device='cuda')
        for i,grp in enumerate(self.group_masks):
            global_group_ids[grp] = i
        from cuml.neighbors import NearestNeighbors
        model = NearestNeighbors(n_neighbors=self.N)
        means = self.sms_model.means.detach().cpu().numpy()
        model.fit(means)
        ...
        ids = torch.arange(self.sms_model.num_points,dtype=torch.long,device='cuda')[group_mask].unsqueeze(-1).repeat(1,self.N)
