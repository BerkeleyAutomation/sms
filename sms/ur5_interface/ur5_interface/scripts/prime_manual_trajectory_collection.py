...
capture = True
append = True
save_path = "/home/lifelong/sms/sms/ur5_interface/ur5_interface/outputs/prime_ellipsoid_new.npy"
...
if capture:
    ...
    import pdb
    pdb.set_trace()
    start_joints = np.array([-1.1512053648578089, -1.1789520422564905, -2.3613441626178187, -1.6598456541644495, 1.7774171829223633, 3.5112481117248535])
    robot.move_joint(start_joints,vel=1.0,acc=0.1)
    time.sleep(0.1)
    curr_pose = robot.get_pose()
    move_left = np.array([0.1,0,0])
    move_forward = np.array([0,-0.1,0])
    move_right = np.array([-0.1,0,0])
    move_backward = np.array([0,0.1,0])
    move_up = np.array([0,0,0.2])
    move_down = np.array([0,0,-0.01])
    left_rotation = np.array([[np.sqrt(2)/2,-np.sqrt(2)/2,0],
                              [np.sqrt(2)/2,np.sqrt(2)/2,0],
                              [0,0,1]])
    second_right_rotation = R.from_euler('z', np.pi/4).as_matrix()
    third_left_rotation = R.from_euler('z', -np.pi/4).as_matrix()
    down_rotation = R.from_euler('x', np.pi/6).as_matrix()
    slight_down_rotation = R.from_euler('x', np.pi/18).as_matrix()
    while True:
        key = get_key()
        if key == 'w': # Move forward
            new_pose = curr_pose
            new_pose.translation += move_forward
            import pdb
            pdb.set_trace()
            robot.move_pose(new_pose,vel=1.0,acc=0.1)
            time.sleep(1.0)
        elif key == 'a': # Move left
            new_pose = curr_pose
            new_pose.translation += move_left
            import pdb
            pdb.set_trace()
            robot.move_pose(new_pose,vel=1.0,acc=0.1)
            time.sleep(1.0)
            ...
            new_pose = curr_pose
            new_pose.translation += move_backward
            import pdb
            pdb.set_trace()
            robot.move_pose(new_pose,vel=1.0,acc=0.1)
            time.sleep(1.0)
            ...
            new_pose = curr_pose
            new_pose.translation += move_right
            import pdb
            pdb.set_trace()
            robot.move_pose(new_pose,vel=1.0,acc=0.1)
            time.sleep(1.0)
        elif key == 'e': # First level left rotation
            new_pose = curr_pose
            new_pose.rotation = new_pose.rotation @ left_rotation
            import pdb
            pdb.set_trace()
            robot.move_pose(new_pose,vel=1.0,acc=0.1)
            time.sleep(1.0)
        elif key == 'r': # Second level right rotation
            new_pose = curr_pose
            new_pose.rotation = second_right_rotation @ new_pose.rotation
            import pdb
            pdb.set_trace()
            robot.move_pose(new_pose,vel=1.0,acc=0.1)
            time.sleep(1.0)
        elif key == 't': # Third level left rotation
            new_pose = curr_pose
            new_pose.rotation = third_left_rotation @ new_pose.rotation
            import pdb
            pdb.set_trace()
            robot.move_pose(new_pose,vel=1.0,acc=0.1)
            time.sleep(1.0)
            ...
            new_pose = curr_pose
            new_pose.translation += move_down
            import pdb
            pdb.set_trace()
            robot.move_pose(new_pose,vel=1.0,acc=0.1)
            time.sleep(1.0)
        elif key == 'k': # Rotate down
            new_pose = curr_pose
            new_pose.rotation = new_pose.rotation @ down_rotation
            import pdb
            pdb.set_trace()
            robot.move_pose(new_pose,vel=1.0,acc=0.1)
            time.sleep(1.0)
        elif key == 'l': # Slight Rotate down
            new_pose = curr_pose
            new_pose.rotation = new_pose.rotation @ slight_down_rotation
            import pdb
            pdb.set_trace()
            robot.move_pose(new_pose,vel=1.0,acc=0.1)
            time.sleep(1.0)
    ...
    # while True:
    #     new_pose = curr_pose
    #     new_pose.rotation = new_pose.rotation @ rotation
    #     import pdb
    #     pdb.set_trace()
    #     robot.move_pose(new_pose,vel=1.0,acc=0.1)
    #     time.sleep(1.0)
    ...
    if append and os.path.exists(save_path):
        joints = np.load(save_path, allow_pickle=True).tolist()
        joints = joints[:len(joints)-1]
        breakpoint()
    else:
        joints = []
...
if capture:
    np.save(save_path, joints_np, allow_pickle=True)